	...
	if fases_concluidas:				
		for fase_concluida in fases_concluidas:
			if fase_concluida.nome == nome_fase:
				if PlayerData.player_1.pontuacao > fase_concluida.pontuacao_maxima:					
					fase_concluida.pontuacao_maxima =PlayerData.player_1.pontuacao
...
func criar_nova_fase_concluida(nome:String) -> void:
	...
	var nova_fase_concluida: Dictionary = {
		...
		"pontuacao_maxima":PlayerData.player_1.pontuacao	
